...
do
    ...
    if ! group_id_exists "$GID"; then
        ...
        add_user_to_group develop "grp$GID"
    else
        add_user_to_group develop "$(get_group_name_from_group_id "$GID")"
    fi
done
